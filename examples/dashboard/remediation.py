    ...
    if "agent_recommendation" in st.session_state:
        ...
        with col2:
            if st.button("🛠️ Execute: Apply Feature Drop & Retrain", use_container_width=True):
                ...
                execute_retrain(drop_features=top_shifted_features)


def execute_retrain(drop_features=None):
    """Trigger the actual backend training pipeline on the current system."""
    project_root = Path(__file__).parent.parent.parent
    # Use the NEW retrain script that trains on expanded data
    script_path = project_root / "examples" / "quickstart" / "retrain_demo.py"
    results_path = project_root / "outputs" / "remediation_results.json"

    # Use the .venv Python so drift_autopsy is importable
    venv_python = project_root / ".venv" / "Scripts" / "python.exe"
    python_exe = str(venv_python) if venv_python.exists() else sys.executable

    st.info(f"🚀 Starting retraining using: `{python_exe}`")
    st.info("📊 **Strategy:** Training on 2014+2015+2016 data → Testing on 2017 & 2018")
